...
class BucketLoaderDataset:
    ...
    def get_dataset(self):
        def get_data_format():
            dataset_path = self.__dataset_URL
        ...
        def get_dataloader_from_local():
            ...
            try:
                if self.from_bucket == True:
                    ...
                    # dataloader = getattr(import_module(".".join([os.getenv("FILES_PATH_BIAS"), "dataloader.dataloader_def"])), class_name)( path_to_dataset)
                else:
                    dataloader = getattr(import_module(self.request['dataloader']['definition']['path']), class_name)(path_to_dataset)
            except KeyError:
...
"Values": list}
        ...
        for col in self.dataframe.columns:
            ...
            feature = {}
            feature["name"] = col
            feature["is_categorical"] = self.dataframe[col].dtype == 'object'
            if feature["is_categorical"]:
                feature["Values"] = self.dataframe[col].unique().tolist()
    ...
    def compatible_metrics(self):
        metrics = []
        dict_metrics= {
            "equalized_odds": "Ensures that the true positive rate (TPR) and false positive rate (FPR) are equal across different groups.",
            "tpr_1": "True Positive Rate for group 1, which measures the proportion of actual positives that are correctly identified.",
            "tpr_0": "True Positive Rate for group 0, which measures the proportion of actual positives that are correctly identified.",
            "fpr_1": "False Positive Rate for group 1, which measures the proportion of actual negatives that are incorrectly identified as positives.",
            "fpr_0": "False Positive Rate for group 0, which measures the proportion of actual negatives that are incorrectly identified as positives."
        }
        for k,v in dict_metrics.items():
            metrics.append({"name": k, "description": v})
        return metrics
    def compatible_mitigations(self):
        """
        returns json:
...
FairUS- FairUS leverages Adversarial Network (CTGAN) to synthesize upsampled data. Unlike traditional approaches that focus solely on balancing subgroup sample sizes, FairUS strategically optimizes the quantity of synthesized data.
        ...
        for k,v in dict_mitigations.items():
            mitigations.append({"name": k, "description": v})
