...
@router.get("/bias_validate_status/{job_id}")
async def get_validation_status(job_id: str):
    job_id = job_id.strip("'")
    bias_validation_status = get_from_db(project_id=PROJECT_ID,
                                    database=FIRESTORE_DB,
                                    collection_name=FIRESTORE_BIAS_VAL_STATUS_COLLECTION,
                                    document_id=job_id)
    if bias_validation_status is None:
        raise HTTPException(status_code=404, detail="Job ID not found")
    return JSONResponse(content=bias_validation_status)