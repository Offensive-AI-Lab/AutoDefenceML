...
@router.get("/dataset_evaluate_status/{job_id}")
async def get_evaluation_status(job_id: str):
    job_id = job_id.strip("'")
    data_eval_status = get_from_db(project_id=PROJECT_ID,
                                    database=FIRESTORE_DB,
                                    collection_name=FIRESTORE_DATA_EVAL_STATUS_COLLECTION,
                                    document_id=job_id)
    if data_eval_status is None:
        raise HTTPException(status_code=404, detail="Job ID not found")
    return JSONResponse(content=data_eval_status)